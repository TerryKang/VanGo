 ...
 <head>
  ...
  <script>		
		...
		function validateUserName() {
			var userName = $$('userName', document).value;
			if(userName == null || userName == '') {
				$$('errUserName', document).innerHTML = "<span class=msg>Username must not be blank.</span>";
				return false;
			} else {
		...
		<div class="findpassword_content">
			<form id="findPassForm" name="findPassForm" action="findPasswordDo.php" method="post" onsubmit="return validateForm()">
				<table class="content2">
					...
					<tr>
						<td class="postheader2"><label for="userName">Username</label></td>							
						<td class="post">
							<input type="text" name="userName" id="userName" maxlength="30" size="30" required>
